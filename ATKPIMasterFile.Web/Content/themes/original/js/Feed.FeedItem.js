atkpimfApp.fn.askedPhoto = function (userId, interestId, element) {
    atkpimfApp.viewModel.sendAskPhoto(userId, interestId)
    // localStorage["askPhotoInf" + userId + "-" + interestId] = "asked";
    $("#feed-add-new-item").css("background", "none");
    $("#feed-add-new-text").text(atkpimfApp.data.localization["Request_sent"]);
    element.removeAttribute("onclick");
    element.classList.contains("pointer") ? element.classList.remove("pointer") : "";
};

atkpimfApp.fn.feedPhotoLoaded = function (img, type) {
...
atkpimfApp.fn.feedGifIndicator = function (indicator) {
    var img;
    var data;
