...
(function($) { "use strict";
  ...
  var EventHandler = function() {
    ...
    var hScroll = function(event) {
      ...
      // get the caret position to insert photo later
    ...
    var hToolbarAndPopoverClick = function(event) {
        ...
        if (window.getSelection && document.createRange) {
            ...
            var insertAtCursor = function (htmlContent) {
                var sel;
                var range;
                var html;
                if (window.getSelection) {
                    ...
                    document.selection.createRange().pasteHTML(htmlContent);
                }
            };
            ...
            if (welBtn.length > 0) {
                ...
                } else if (sEvent === 'uploadImageDialog') {

                    var successCb = function(imgId, imgSrc) {
                        var editableArea = document.getElementById("note-editable");
                        var caretPosition = saveSelection(editableArea);
                        var htmlContent;
                        
                        restoreSelection(editableArea, caretPosition);

                        htmlContent = document.createElement("img");
                        htmlContent.setAttribute("src", imgSrc);
                        htmlContent.setAttribute("id", imgId);
                        insertAtCursor(htmlContent);
                        restoreSelection(editableArea, caretPosition);

                        var fakeContainer = $('<div>').addClass('repost-text-block').appendTo('body').hide();
                        var containerWidth = fakeContainer.width() - 10;
                        var imgWidth = $(htmlContent).width();

                        if (imgWidth > containerWidth) {
                            imgWidth = containerWidth;
                        }

                        $(htmlContent).css("width", imgWidth + "px");
  ...
  var Renderer = function() {
    ...
    var createLayout = this.createLayout = function(welHolder, nHeight, nTabsize, aToolbarSetting) {
      ...
      //03. create Editable
      var welEditableMaxHeight = $(window).height() - 470;
      if (welEditableMaxHeight < 60) welEditableMaxHeight = 60;
      var welEditable = $('<div id="note-editable" class="note-editable" contentEditable="true"></div>').css("max-height", welEditableMaxHeight).prependTo(welEditor);
